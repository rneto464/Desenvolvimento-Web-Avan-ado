document.addEventListener('DOMContentLoaded', () => {
  ...
  // Buscar lista de regiões para o menu
  async function fetchRegions() {
    try {
      const response = await fetch('/api/regions');
      const regions = await response.json();
      
      // Criar botão default para "Todas as Notícias" (Master)
      ...
      // Carregar notícias da região default
      fetchNews('all', 'Todas');
    } catch (error) {
  ...
  function renderNews(articles) {
    ...
    articles.forEach((article, index) => {
      ...
      // Formatando Markdown simples de volta para texto no resumo se necessário (o backend manda em markdown)
